...
import 'package:retroachievements_library/screens/consoles/components/consoles_filters.dart';
import 'package:retroachievements_library/screens/consoles/components/consoles_grid.dart';
import 'package:retroachievements_library/screens/consoles/components/consoles_header.dart';
import 'package:retroachievements_library/screens/consoles/components/consoles_list.dart';
...
class _GamesContentState extends ConsumerState<GamesContent>
    with AutomaticKeepAliveClientMixin {
  ...
  Future<void> _loadConsoleStats() async {
    try {
      ...
      for (final consoleId in supportedConsoleIds) {
        ...
        final cachedTotals = await localDataRepository.getConsoleTotals(
          consoleId,
        );
        ...
        if (cachedTotals != null) {
          ...
          ref
              ...
              .updateConsoleStats(consoleId, stats);
  ...
  Future<void> _preloadGameData() async {
    ...
    Future.microtask(() {
      if (mounted) {
        ref.read(consolesUpdatingStateProvider.notifier).state = true;
      }
    });
    ...
    try {
      ...
      for (int i = 0; i < supportedConsoleIds.length; i += batchSize) {
        final end =
            (i + batchSize < supportedConsoleIds.length)
                ? i + batchSize
                : supportedConsoleIds.length;
          ...
          batch.map((consoleId) async {
            try {
              ...
              if (cachedTotals == null) {
                ...
                final gamesListRaw = await repository.getGameList(
                  consoleId.toString(),
                  apiKey,
                  useCache: true,
                );
                ...
                if (gamesListRaw != null) {
                  ...
                  final gamesList =
                      gamesListRaw
                          ...
                          .where(
                            (game) => !settings.shouldIgnoreGame(game.title),
                          )
                          .toList();
                  ...
                  await localDataRepository.saveConsoleTotals(
                    consoleId,
                    totalGames,
                    totalHashes,
                  );
  ...
  void _refreshData() async {
    ...
    try {
      ...
      await ref
          .read(consolesStateProvider.notifier)
  ...
  List<Console> _getFilteredConsoles() {
    ...
    List<Console> filteredList =
        consoleState.data!
            .where((console) => console.name.toLowerCase() != 'standalone')
            .toList();
    ...
    if (_searchQuery.isNotEmpty) {
      ...
      filteredList =
          filteredList
              .where((console) => console.name.toLowerCase().contains(query))
              .toList();
    }
    ...
    if (statsCache.isNotEmpty && mounted) {
      for (final entry in statsCache.entries) {
        if (!_libraryStats.containsKey(entry.key) ||
            _libraryStats[entry.key]!['lastUpdated'] !=
