...
class UserStatsRepositoryImpl implements UserStatsRepository {
  ...
  Future<Map<String, dynamic>> getUserCompletionProgressRaw(
    ...
    final response = await _allCompletionApi.getUserCompletionProgress(
      username,
      apiKey,
    );
  ...
  Future<Map<String, dynamic>?> getUserCompletionProgress(
    ...
    final response = await getUserCompletionProgressRaw(
      username,
      apiKey,
      useCache: useCache,
    );
  ...
  @override
  Future<Map<String, dynamic>?> getCachedCompletionProgress(
    String username,
  ) async {
  ...
  Future<Map<String, dynamic>> getUserCompletedGamesRaw(
    ...
    final response = await _completedGamesApi.getUserCompletedGames(
      username,
      apiKey,
    );
    if (response['success'] && response['data'] != null) {
  ...
  Future<List<CompletedGame>?> getUserCompletedGames(
    ...
    final response = await getUserCompletedGamesRaw(
      username,
      apiKey,
      useCache: useCache,
    );
  ...
  Future<List<dynamic>?> getCachedCompletedGames(String username) async {
    final data = await _storageService.readJsonData(
      'completed_games',
      username,
    );
    return data?['games'];
  ...
  Future<Map<String, dynamic>> getUserRecentlyPlayedGamesRaw(
    ...
    final response = await _recentlyPlayedApi.getUserRecentlyPlayedGames(
      username,
      apiKey,
      count: count,
    );
  ...
  Future<List<RecentlyPlayedGame>?> getUserRecentlyPlayedGames(
    ...
    final response = await getUserRecentlyPlayedGamesRaw(
      username,
      apiKey,
      count: count,
      useCache: useCache,
    ...
    if (response['success'] && response['data'] != null) {
      final List<dynamic> gamesJson = response['data'];
      return gamesJson
          .map((json) => RecentlyPlayedGame.fromJson(json))
          .toList();
    }
    return null;
  }
  ...
  @override
  Future<void> cacheRecentlyPlayedGames(
    String username,
    List<dynamic> data,
  ) async {
    await _storageService.saveJsonData(
  ...
  Future<UserAwards?> getUserAwards(
    ...
    final response = await getUserAwardsRaw(
      username,
      apiKey,
      useCache: useCache,
    );
  ...
  Future<UserSummary?> getUserSummary(
    ...
    final response = await getUserSummaryRaw(
      username,
      apiKey,
      useCache: useCache,
    );
