...
import 'package:retroachievements_library/repositories/local_data_repository.dart';
...
class LocalDataRepositoryImpl implements LocalDataRepository {
  ...
  Future<void> saveConsoleTotals(
    ...
    try {
      ...
      debugPrint(
        'Console totals saved for console $consoleId: $totalGames games, $totalHashes hashes',
      );
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> getConsoleTotals(int consoleId) async {
    try {
      return await _storageService.readJsonData(
        'console_totals',
        'console_$consoleId',
      );
    } catch (e) {
  ...
  Future<List<Console>> getLocalConsolesFolders() async {
    try {
      ...
      final cachedData = await _storageService.readJsonData(
        'local_data',
        'consolesfolders',
      );
      ...
      if (cachedData != null && cachedData['consolesfolder'] != null) {
        final folders =
            (cachedData['consolesfolder'] as List)
      ...
      final jsonString = await rootBundle.loadString(
        'assets/data/consolesfolders.json',
      );
      final jsonData = json.decode(jsonString) as Map<String, dynamic>;

      if (jsonData['consolesfolders'] != null) {
        final folders =
            (jsonData['consolesfolders'] as List)
                .map((consoleJson) => Console.fromJson(consoleJson))
                .toList();

        // Cache the data for future use
        await _storageService.saveJsonData(
          {'consoles': jsonData['consoles']},
          'local_data',
          'consoles',
        );
  ...
  Future<void> cleanHashesForRemovedFolders(
    int consoleId,
    ...
    try {
      ...
      if (gamesList != null && gamesList.isNotEmpty) {
        ...
        for (final game in gamesList) {
          ...
          if (ignoreHomebrew && title.contains('~Homebrew~')) {
            shouldIgnore = true;
          }
          if (ignorePrototype && title.contains('~Prototype~')) {
            shouldIgnore = true;
          }
          if (ignoreUnlicensed && title.contains('~Unlicensed~')) {
            shouldIgnore = true;
          }
      ...
      debugPrint(
        'Cleaned up ${currentHashes.length - updatedHashes.length} hashes from removed folders',
      );
      debugPrint(
        'Updated stats: $matchedGamesCount games, $matchedHashesCount hashes',
      );
    } catch (e) {
  ...
  Future<void> saveHashStats(
    ...
    try {
      ...
      debugPrint(
        'Hash stats saved for console $consoleId: $matchedGames games, $matchedHashes hashes',
      );
    } catch (e) {
            ...
            await for (final entity in directory.list(recursive: true)) {
              if (entity is File) {
                ...
                if (validExtensions.contains(extension) &&
                    !existingHashes.containsKey(entity.path)) {
